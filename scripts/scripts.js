document.addEventListener("DOMContentLoaded", function () {
  ...
  const email = "victorshliu@gmail.com";
const link = document.getElementById("email-link");
link.href = "mailto:" + encodeEmail(email);

function encodeEmail(email) {
  // Split the email address into username and domain
  const [username, domain] = email.split("@");

  // Replace periods and at symbols with their corresponding HTML entities
  const encodedUsername = username.replace(/\./g, "&#46;").replace(/@/g, "&#64;");
  const encodedDomain = domain.replace(/\./g, ".");

  // Return the encoded email address as a string
  return encodedUsername + "@" + encodedDomain;
}
